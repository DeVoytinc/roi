...
class InitiativeScreen extends StatelessWidget {
  const InitiativeScreen({super.key, required this.index,});
      ...
      body:  Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              IconButton(onPressed: (){goBack(context);}, icon: Icon(Icons.arrow_back)),
              Padding(
                      ...
                      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
                        if (snapshot.hasData){
                          return Wrap(
                            ...
                            runSpacing: 10,
                            children: 
                              List.generate(curInitiative.categorytitle.length, (index) {
                                return Container(
                                  decoration: BoxDecoration(
                                    borderRadius: BorderRadius.circular(20),
              ...
              FutureBuilder(
                ...
                builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
                  if (!snapshot.hasData){
                    return const CircularProgressIndicator(color: Color.fromARGB(255, 190, 36, 36),);
                  }
                  else {
                    return Padding(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox( width: 10,),
                              Text('${curInitiative.datepollbegin.toUnixTime().toString().substring(0, 10)} - ${curInitiative.datepollend.toUnixTime().toString().substring(0, 10)}',)
                            ],
                          ),
                          const SizedBox(height: 20,),
                          TextButton(
                            onPressed: () => launchUrl(Uri.parse(curInitiative.url)), 
                            child: const Text(
